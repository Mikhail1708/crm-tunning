...
export const Categories = () => {
  ...
  const handleFieldSubmit = async (e) => {
    ...
    const fieldData = {
      ...
      sortOrder: fieldFormData.sortOrder || 0,
  ...
  // Открыть модальное окно редактирования характеристики
  const openEditFieldModal = (field) => {
    setEditingField(field);
    // Парсим options если есть
    let optionsArray = [];
    if (field.options) {
      try {
  ...
  // Открыть модальное окно добавления характеристики
  ...
  const fieldTypes = [
    { value: 'select', label: 'Выпадающий список (один выбор)', icon: '📋' },
    { value: 'multiselect', label: 'Множественный выбор', icon: '✅' },
    { value: 'text', label: 'Текстовое поле', icon: '📝' },
    { value: 'number', label: 'Числовое поле', icon: '🔢' }
  ];
          ...
          return (
            <Card key={category.id} className="hover:shadow-lg transition-shadow">
              <CardBody className="p-6">
                ...
                {/* Характеристики категории - всегда видимые */}
